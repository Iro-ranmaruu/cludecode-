...
export default async function ProductMasterPage({
  ...
  const allRows = listProductMaster();
  const rows = query
    ? allRows.filter(
        (r) =>
          r.makerCode.includes(query) ||
          r.productCode.includes(query) ||
          r.productName.includes(query)
      )
    : allRows;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-1 text-sm text-slate-600">
        メーカーコード＋商品コードをキーに、商品名・梱包単位・通常仕切（営業仕切）・特価目安を登録します。
        特価申請・不具合処理依頼・サンプル依頼の各フォームで、この情報が自動反映されます
        （特価申請では通常仕切・梱包単位が自動入力され、特価目安が参考表示されます）。
      </p>

      <section className="mt-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-base font-semibold text-slate-900">CSV一括インポート</h2>
        <p className="mt-1 text-xs text-slate-500">
          1行につき「メーカーコード,商品コード,商品名,梱包単位,営業仕切,特価目安」の順で貼り付けてください（ヘッダー行があっても構いません。営業仕切・特価目安は省略可）。既存の組み合わせは上書きされます。
        </p>
        <form action={importProductMasterCsvAction} className="mt-3 space-y-3">
          <textarea
            ...
            rows={6}
            placeholder={
              "メーカーコード,商品コード,商品名,梱包単位,営業仕切,特価目安\n71168,WDM2-W,WiSMディスポマスクII,箱,500,420"
            }
      ...
      <section className="mt-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        >
          ...
          <div>
            <label className={labelCls}>通常仕切（営業仕切）</label>
            <input type="number" step="any" name="standardWholesalePrice" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>特価目安</label>
            <input type="number" step="any" name="guidelinePrice" className={inputCls} />
          </div>
      ...
      <section className="mt-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-slate-900">
            登録済みマスタ（全{allRows.length}件{query ? `、検索結果${rows.length}件` : ""}）
          </h2>
          <form method="get" className="flex gap-2">
            <input
              ...
              name="q"
              defaultValue={query}
              placeholder="メーカーコード・商品コード・商品名で検索"
          ...
          <>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-2">通常仕切</th>
                    <th className="px-4 py-2">特価目安</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {displayRows.map((r) => (
                    <tr key={`${r.makerCode}-${r.productCode}`}>
                      ...
                      <td className="px-4 py-2">{formatNumber(r.standardWholesalePrice)}</td>
                      <td className="px-4 py-2">{formatNumber(r.guidelinePrice)}</td>
            ...
            {rows.length > displayRows.length && (
              <p className="mt-2 text-xs text-slate-500">
                最初の{displayRows.length}件のみ表示しています。絞り込むには検索をご利用ください。
              </p>
            )}
          </>
        )}
